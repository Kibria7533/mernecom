     ...
     setOpenhandeler=()=>{
      ...
      megamenuhandel=(data)=>{
       {this.state.data.map(item=>{
         if(item.CategoryName===data && item.SubCategory.length && this.state.clicked===item.CategoryName ){
           this.setState({megadata:item.SubCategory,showstate:!this.state.showstate,clicked:item.CategoryName})
         }
        else if(item.CategoryName===data && item.SubCategory.length && this.state.clicked!=item.CategoryName ){
          this.setState({megadata:item.SubCategory,showstate:true,clicked:item.CategoryName})
        }
  ...
  await  axios.get(`${URL}/getmenus`, {
      ...
      const mydata=data.data.map((item,index)=>{
        if(!item.SubCategory.length){
         var info={
          content:[{id:index,name:item.CategoryName,to:`/${item.CategoryName}`}]
         }
         return info;
        }
        else{
          ...
          var info={
            content:[{id:index,name:item.CategoryName,children:children}]
           }
           return info;
                    ...
                    {this.state.showstate &&
                        <ul style={{ marginTop: "40px" }}>
                            <FlatList
                                ...
                                renderWhenEmpty={() => <div>You didn't add any category yet</div>}
                                display={{
                                    grid: true,
                                    minColumnWidth: "10px",
                                    gridGap: "5px"
                            ...
                            <ul className="navbar-nav ml-auto ">
                                {this.state.data.map((item, index) => {
                                    if (item.Type === "megamenu") {
                                        return (
                                            <li key={index} className="nav-item dropdown has-megamenu">
                                                <a className="nav-link dropdown-toggle" id="my" href="#" on onClick={() => this.megamenuhandel(item.CategoryName)}>{item.CategoryName}</a>
                                            </li>
                                        )
                                    }
...
<span className='badge badge-warning' id='lblCartCount'>{this.props.wishlistnumber} </span>
                            ...
                            <span className='badge badge-warning' id='lblCartCount'> {this.props.addtocart}</span>
                                   </li> 
                          {this.state.auth?
                           <li>
                           <Avatar name={this.state.username} size="30" style={{"paddingTop":"8px"}}  onClick={this.openrightcanvas} maxInitials={1} round textSizeRatio={2.75} />
                           </li>: <li className="nav-item dropdown">
                                                <a className="nav-link  dropdown-toggle" data-toggle="dropdown"><i className="fa fa-user-circle-o" aria-hidden="true"></i></a>
                                                <ul className="dropdown-menu dropdown-menu-right">
                                                   
                                                      <li><a className="dropdown-item" href="/userregister">Register</a></li>
...
<span className='badge badge-warning' id='lblCartCount' style={{    marginTop: "33px"}}>{this.props.addtocart}</span>
                    ...
                    {this.state.auth?
                          ...
                          : <li className="nav-item dropdown list-unstyled" >
                                                <a className="nav-link  dropdown-toggle" data-toggle="dropdown"><i className="fa fa-user-circle-o" style={{"fontSize":"30px"}} aria-hidden="true"></i></a>
                                                <ul className="dropdown-menu dropdown-menu-right">
