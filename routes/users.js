...
router.post("/register-user", body('password_confirmation').custom((value, { req }) => {
  ...
  .isLength({ min: 5 }),
  check('address').not().isEmpty().withMessage('address can not be null').isLength({
    min: 3
  })
...
router.get('/active/:activeToken', function (req, res, next) {
  ...
  User.findOne({
    ...
    user.confirmed = true;
    user.save().then(ok=>{
      res.redirect(`${process.env.URL}/userlogin`);
    })
...
router.post('/subforgotpasswordforms/:forgotpasswordtoken', body('password_confirm').custom((value, { req }) => {
  if (value !== req.body.password) {
    throw new Error('Password confirmation does not match password');
  }
  return true;
 ...
 await User.findOne({ activeToken: req.params.forgotpasswordtoken }).then(async(user)=>{
